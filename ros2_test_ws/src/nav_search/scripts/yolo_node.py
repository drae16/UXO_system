...
class YoloDetectNode(Node):
    ...
    def spatial_transformation(self,pos,knownParamType, knownParamValue):
        ...
        cam = ct.Camera(ct.RectilinearProjection(focallength_px=self.FOCAL_LENGTH, 
                         ...
                         ct.SpatialOrientation(pos_x_m = self.POS_X,
                                               pos_y_m = self.POS_Y,
    ...
    def execute_cb(self, goal_handle):
        ...
        # 1) YOLO detect
        results = self.model.predict(img,show=True,conf=min_conf)[0]
        boxes = results.boxes
        ...
        try:
            tf = self.tf_buffer.lookup_transform(
                "map", "vx300s/camera_link", rclpy.time.Time())
            q_2 = tf.transform.rotation
            position = tf.transform.translation
            q_2x, q_2y, q_2z, q_2w = q_2.x, q_2.y, q_2.z, q_2.w

            _, _, heading_map = euler_from_quaternion([q_2x, q_2y, q_2z, q_2w])
        ...
        #estimate the target orientation here
        dx = x_max - x_min
        dy = -(y_max - y_min)   # flip image y to math y
        theta = np.mod(np.arctan2(dy, dx), np.pi)

        orientation_est = (math.pi/2-theta) +heading_map
        ...
        self.get_logger().info(f"Total distance to target = {np.sqrt(x_base**2 + y_base**2)}, axis = {orientation_est}")
        result.found = True
        result.x_base = float(y_base)
        result.y_base = float(-1*x_base)
        result.axis = orientation_est
        result.confidence = best_conf
