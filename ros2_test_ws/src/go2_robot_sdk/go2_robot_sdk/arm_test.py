...
from nav_search.action import RunSearchTree


class ArmTest(Node):
    def __init__(self):
        super().__init__("gps_node")
        self.test_client = ActionClient(self, RunSearchTree, '/run_search_tree')
        while not self.test_client.wait_for_server(timeout_sec=2.0):
            self.get_logger().info('Waiting for tree to start..')
    ...
    def call_scan_area(self):
        ...
        goal = RunSearchTree.Goal()
        goal.scan_start_angle = -3.14
        goal.scan_end_angle = 3.14
        goal.scan_steps = 10
        ...
        if result.succeeded:
            self.get_logger().info(
                f'ScanArea: target found at base (x={result.final_x_map:.2f}, y={result.final_y_map:.2f}, distance: {result.final_distance:.2f})'
            )
        else:
